...
<script>
    ...
    function addTrack() {
        TracksArray.update(arr => {
            ...
            return [  
                ...arr, 
                {id: nextId, instrument: "keys", sample: null, instrumentIcon: keys, color: "#00bf00", volume: 50, pan: 50, panAngle:0, muted: false, regions: []}  //allow user to choose instrument before adding a track (do later)
            ]
        })
